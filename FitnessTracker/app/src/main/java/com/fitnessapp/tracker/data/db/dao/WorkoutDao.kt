...
interface WorkoutDao {
    @Query("SELECT * FROM workouts WHERE isDraft = 0 AND id IN (SELECT DISTINCT workoutId FROM workout_sets) ORDER BY date DESC")
    fun getAllWorkouts(): Flow<List<WorkoutEntity>>

    @Query("SELECT * FROM workouts WHERE isDraft = 0 AND date >= :start AND date <= :end AND id IN (SELECT DISTINCT workoutId FROM workout_sets) ORDER BY date ASC")
    fun getWorkoutsInRange(start: Long, end: Long): Flow<List<WorkoutEntity>>

    @Query("SELECT * FROM workouts WHERE isDraft = 0 AND date >= :dayStart AND date <= :dayEnd AND id IN (SELECT DISTINCT workoutId FROM workout_sets) ORDER BY date ASC")
    suspend fun getWorkoutsByDay(dayStart: Long, dayEnd: Long): List<WorkoutEntity>

    @Query("SELECT * FROM workouts WHERE isDraft = 0 AND date = :date AND id IN (SELECT DISTINCT workoutId FROM workout_sets) LIMIT 1")
    ...
    @Query("SELECT COUNT(*) FROM workouts WHERE isDraft = 0 AND date >= :start AND date <= :end AND id IN (SELECT DISTINCT workoutId FROM workout_sets)")
    suspend fun getWorkoutCountInRange(start: Long, end: Long): Int

    @Query("SELECT COUNT(*) FROM workouts WHERE isDraft = 0 AND id IN (SELECT DISTINCT workoutId FROM workout_sets)")
    suspend fun getTotalWorkoutCount(): Int

    @Query("SELECT COALESCE(SUM(CAST((endTime - startTime) AS REAL)), 0) FROM workouts WHERE isDraft = 0 AND date >= :start AND date <= :end AND id IN (SELECT DISTINCT workoutId FROM workout_sets)")
    ...
    @Query("SELECT * FROM workout_sets WHERE exerciseId = :exerciseId AND workoutId IN (SELECT id FROM workouts WHERE isDraft = 0) AND weight IS NOT NULL ORDER BY weight DESC, reps DESC LIMIT 1")
    suspend fun getMaxWeightSetForExercise(exerciseId: Long): WorkoutSetEntity?

    @Query("SELECT * FROM workout_sets WHERE exerciseId = :exerciseId AND workoutId IN (SELECT id FROM workouts WHERE isDraft = 0) AND reps IS NOT NULL ORDER BY reps DESC LIMIT 1")
    suspend fun getMaxRepsSetForExercise(exerciseId: Long): WorkoutSetEntity?

    @Query("SELECT * FROM workout_sets WHERE exerciseId = :exerciseId AND workoutId IN (SELECT id FROM workouts WHERE isDraft = 0) AND durationSeconds IS NOT NULL ORDER BY durationSeconds DESC LIMIT 1")
    suspend fun getMaxDurationSetForExercise(exerciseId: Long): WorkoutSetEntity?

    @Query("SELECT ws.id, ws.workoutId, w.date, w.note as workoutNote, ws.exerciseId, e.name as exerciseName, e.bodyPart as bodyPart, ws.setNumber, ws.recordType, ws.weight, ws.reps, ws.durationSeconds, ws.restSeconds FROM workout_sets ws INNER JOIN workouts w ON ws.workoutId = w.id INNER JOIN exercises e ON ws.exerciseId = e.id WHERE w.isDraft = 0 AND w.date >= :dayStart AND w.date < :dayEnd ORDER BY w.date ASC, ws.workoutId ASC, ws.exerciseId ASC, ws.setNumber ASC")
    suspend fun getWorkoutSetsWithExerciseByDay(dayStart: Long, dayEnd: Long): List<WorkoutSetWithExercise>

    @Query("SELECT ws.id, ws.workoutId, w.date, w.note as workoutNote, ws.exerciseId, e.name as exerciseName, e.bodyPart as bodyPart, ws.setNumber, ws.recordType, ws.weight, ws.reps, ws.durationSeconds, ws.restSeconds FROM workout_sets ws INNER JOIN workouts w ON ws.workoutId = w.id INNER JOIN exercises e ON ws.exerciseId = e.id WHERE w.isDraft = 0 ORDER BY w.date ASC, ws.workoutId ASC, ws.exerciseId ASC, ws.setNumber ASC")
    suspend fun getAllWorkoutSetsWithExercise(): List<WorkoutSetWithExercise>

    @Query("SELECT e.bodyPart as bodyPart, COUNT(ws.id) as count FROM workout_sets ws INNER JOIN exercises e ON ws.exerciseId = e.id INNER JOIN workouts w ON ws.workoutId = w.id WHERE w.isDraft = 0 GROUP BY e.bodyPart")
    suspend fun getBodyPartSetsCount(): List<BodyPartCount>
    ...
    @Query("SELECT ws.id, ws.workoutId, ws.exerciseId, ws.setNumber, ws.recordType, ws.weight, ws.reps, ws.durationSeconds, ws.restSeconds, w.date as date FROM workout_sets ws INNER JOIN workouts w ON ws.workoutId = w.id WHERE ws.exerciseId = :exerciseId AND w.isDraft = 0 ORDER BY w.date DESC, ws.id DESC")
    suspend fun getSetsForExerciseWithDate(exerciseId: Long): List<WorkoutSetWithDate>
}
